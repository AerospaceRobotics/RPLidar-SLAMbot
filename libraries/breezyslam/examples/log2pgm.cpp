...
static const char * DATASET                     = "exp2";

// Flags
static const bool USE_ODOMETRY                  = true;
static const bool USE_PARTICLE_FILTER           = true;

// CoreSLAM params: gives us a nice-size map
static const int MAP_SIZE_PIXELS                = 800;
static const double MAP_SCALE_PIXELS_PER_METER  = 30;
...
public:
    ...
    Rover() : WheeledRobot(
        0.077,      // wheelRadiusMeters
        0.165)      // halfAxleLengthMeters
    {
    }
    
    void computeVelocities(long * odometry, double & dxy, double & dtheta, double & dt)
    {  
        WheeledRobot::computeVelocities(odometry[0], odometry[1], odometry[2], 
            dxy, dtheta, dt);
    }
...
int meters2pixels(double c)
{
    return (int)(c * MAP_SCALE_PIXELS_PER_METER);
}


ProgressBar * make_progress_bar(int maxval)
{
    return new ProgressBar(0, maxval, 80);
}

void show_progress_bar(ProgressBar * progbar, int amt)
{
...
Position update_with_odometry(CoreSLAM & slam, 
    int * scan, long * odometry, Rover & robot)
{
    double dxy, dtheta, dt;
    robot.computeVelocities(odometry, dxy, dtheta, dt);
    return slam.update(scan, dxy, dtheta, dt);
}
...
{    
    ...
    // Create SLAM object with Hokuyo HRG04 laser
    HokuyoURG04 laser;
    CoreSLAM slam = CoreSLAM(laser, 
        MAP_SIZE_PIXELS, MAP_SCALE_PIXELS_PER_METER, RANDOM_SEED);
    ...
    printf("Processing %d scans with%s odometry / with%s particle filter...\n",
        nscans, USE_ODOMETRY ? "" : "out", USE_PARTICLE_FILTER ? "" : "out");
    ProgressBar * progbar = make_progress_bar(nscans);
    ...
    {                                       
        int * scan = scans[scanno];
        
        // Get updated position with/out odometry
        Position position = 
        USE_ODOMETRY ? 
        update_with_odometry(slam, scan, odometries[scanno], robot) :
        slam.update(scan);  
        
        // Add new coordinates to trajectory
        double * v = new double[2];
        v[0] = position.x_meters;
        v[1] = position.y_meters;
    ...
    {        
        ...
        int x = meters2pixels(v[0]);
        int y = meters2pixels(v[1]);
